...
export default function EmailQuiz() {
  ...
  if (!questions[currentQuestion]) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
        <ShieldLogo />
        <h1 className="text-2xl font-bold mt-4">Loading next question...</h1>
      </div>
    );
  }
        ...
        {!quizComplete ? (
          <>
            <div className="flex justify-between items-center mb-6">
              <span className="text-md">Question {currentQuestion + 1} of 5</span>
            ...
            <Card className="mb-6">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-500">
              ...
              {[{
                ...
                <Card
                  ...
                  className={`cursor-pointer transition-all hover:shadow-md ${
                    !answered && selectedEmail === item.email ? 'ring-2 ring-blue-500' :
                    answered && selectedEmail === item.email && item.email === currentQ.fake_email ? 'ring-2 ring-green-500' :
                    answered && selectedEmail === item.email && item.email !== currentQ.fake_email ? 'ring-2 ring-red-500' :
                    answered && item.email === currentQ.fake_email && selectedEmail !== item.email ? 'ring-2 ring-green-500' : ''
                  }`}
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <div className="border-t pt-4 whitespace-pre-line">{item.message}</div>
                    <div className="mt-6 pt-4 border-t text-center font-medium">{item.label}</div>
                  </CardContent>
                </Card>
              ))}
            </div>
              ...
              <div className="mb-6">
                {isCorrect ? (
                  <div className="flex items-center p-4 bg-green-100 border border-green-400 rounded">
                    ...
                    <span>
                      Correct! This is a phishing email. Red flags include: the sender domain '{currentQ.fake_email.split('@')[1]}' (not an official domain), creating urgency with a threat, and requesting you to click a suspicious link.
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center p-4 bg-red-100 border border-red-400 rounded">
                    ...
                    <span>
                      Incorrect! The phishing attempt is from '{currentQ.fake_email}'. Red flags include: unofficial domain name, creating urgency, and requesting you to click a suspicious link.
                    </span>
                  </div>
                )}
              </div>
          ...
          <Card className="w-full">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-xl mb-6">Your final score: {score}/5</p>
              <div className="mb-8">
                {score === 5 ? (
