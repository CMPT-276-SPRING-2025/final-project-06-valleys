...
export function SecurityVendorsTable({ results }) {
  ...
  const sortedResults = Object.entries(results)
    .sort(([vendorA, resultA], [vendorB, resultB]) => {
      // Define category priority
      const getPriority = (category) => {
        switch (category) {
          case "malicious": return 1;
          case "suspicious": return 2;
          case "harmless": return 3;
          case "timeout": return 4;
          case "undetected": return 5;
          default: return 6;
        }
      };
            ...
            <div className="max-h-96 overflow-auto">
              ...
              <div className="hidden md:block">
                <table className="w-full">
                  <thead className="bg-gray-50 text-left text-sm font-medium text-gray-500">
                    <tr>
                      <th className="px-4 py-3 w-1/4">Security Vendor</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {Array.from({ length: Math.ceil(sortedResults.length / 2) }).map((_, rowIndex) => {
                      ...
                      return (
                        <tr key={rowIndex} className="hover:bg-gray-50">
                          ...
                          {leftIndex < sortedResults.length && (
                            <>
                              <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900 border-r border-gray-100">
                                {sortedResults[leftIndex][0]}
                              </td>
                              <td className="px-4 py-3 text-sm border-r border-gray-200">
                                {renderBadge(sortedResults[leftIndex][1].category)}
                              </td>
                            </>
                          )}
                          ...
                          {rightIndex < sortedResults.length ? (
                            <>
                              ...
                              <td className="px-4 py-3 text-sm">
                                {renderBadge(sortedResults[rightIndex][1].category)}
                              </td>
                            </>
                          ) : (
