...
test.describe("Email Generator Page", () => {
  ...
  test("should load the email generator page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Phishing Email Generator" })).toBeVisible();
    
    // Check if the disclaimer is present
    await expect(page.getByText("This tool is for educational purposes only")).toBeVisible();
  });
  ...
  test("should update subject and content when template is changed", async ({ page }) => {
  ...
  test("should enable customization mode when Customize is selected", async ({ page }) => {
