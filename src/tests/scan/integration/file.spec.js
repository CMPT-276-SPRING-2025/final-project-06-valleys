...
test.describe('File Scan Page', () => {
  ...
  test('should show error for oversized files', async ({ page }) => {
    ...
    await expect(page.locator('text=File size exceeds the 32MB limit')).toBeVisible();
  });
  ...
  test('should handle API errors gracefully', async ({ page }) => {
    ...
    // or create a temporary file that might cause an error
    ...
    try {
      ...
      // Look for any error message, not just a specific text
